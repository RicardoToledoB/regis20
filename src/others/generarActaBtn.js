...
const policeName = `${form.police?.firstName || ""} ${form.police?.firstLastName || ""}`.trim() || "—"
...
const policeCommune = form.police?.institutionType?.commune?.name || "—"
  ...
  doc.text(
    `Ord N° ${form.of_number} con fecha ${new Date(
      form.of_number_date
    ).toLocaleDateString("es-CL")} - ${(form.police?.institutionType?.institution?.name || "").toUpperCase()} - ${
