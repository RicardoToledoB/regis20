<template>
  <div class="card flex justify-center">
    ...
    >
      <template #header>
        <span class="text-xl font-semibold">Crear Tipo de Contenedor</span>
      </template>
      ...
      <template #footer>
        ...
        <Button label="Guardar" @click="savePackaging" :loading="isLoading"/>
      </template>
    </Dialog>
  </div>
</template>
...
import packagings from '@/services/packagingsService'
  ...
  setup(props, { emit }) {
    ...
    const savePackaging = async () => {
      try {
        ...
        const payload = { ...form }
        const { data } = await packagings.create(payload)
