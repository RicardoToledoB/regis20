<template>
  <div>
    ...
    >
      <template #header>
        <span class="text-xl font-semibold">Editar Ubicación Bodega</span>
      </template>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    watch(
      () => visible.value,
      (isVisible) => {
        if (isVisible && props.storageLocation) {
          form.name = props.storageLocation.name || ''
        }
      }
    )
    ...
    const updateStorageLocation = async () => {
      ...
      try {
        ...
        const { data } = await storagesLocationsService.update(props.storageLocation.id, payload)
